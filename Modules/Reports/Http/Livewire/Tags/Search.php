<?php
	...
	namespace Modules\Reports\Http\Livewire\Tags;
	...
	class Search extends Component {
		...
		public function mount ()
		{
			...
			$this->allTags      = AssignmentsTags::where('active', 'Y')->orderBy('name')->get();
		...
		{
			$date_from = (new \DateTime($this->date_from ?: 'first day of this month'))->format('Y-m-d');
			$date_to   = (new \DateTime($this->date_to ?: 'now'))->format('Y-m-d');
			
			$this->list = AssignmentFinanceRepository::SearchTags($this->tagsSelected);
			...
			switch ( $this->filter_type ) {
				...
				case 'referral':
					switch ( $this->filter_date ) {
						case 'created':
							$this->list = $this->list->DateCreated($date_from, $date_to, $this->techSelected, $this->jtSelected)
							                         ->get();
							break;
						case 'schedulled':
							$this->list = $this->list->DateSchedulled($date_from, $date_to, $this->techSelected, $this->jtSelected)
							                         ->get();
							break;
						case 'billed':
							$this->list = $this->list->DateBilled($date_from, $date_to, $this->techSelected, $this->commissionsSelected)
							                         ->get();
							...
							break;
						case 'paid':
							$this->list = $this->list->DatePaid($date_from, $date_to, $this->commissionsSelected)->get();
							break;
					}
					...
					if ( $this->tagsSelected ) {
						...
						//$this->list=$this->list->whereIn('tags', $this->tagsSelected);
		...
		public function render ()
		{
			...
			return view('reports::livewire.tags.search');
		}
