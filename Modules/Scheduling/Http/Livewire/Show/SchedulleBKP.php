...
class SchedulleBKP extends Component
{
    ...
    public function mount(){
        ...
        $this->techs = Techs::with('users')->get();
...
//                    dd($milhas_value);
    ...
    public function getOpenJobs ($jobs, $city,$state){

        if(count($jobs) > 0){
            foreach ($jobs as $job){
//dd("$this->originAddress, $job->originAddress");
//                    $milhas = $this->getMiles($this->originAddress, $job->originAddress);
//                    $milhas_text=$milhas->text;
//                    $milhas_value=$milhas->value;
...
//        dd($direction);
    ...
    public function dateJobs (){
        $this->schedulled = AssignmentsScheduling::whereDate('start_date', $this->date)->get();
    }
...
//        dd($check);
//
////        $check = $check->whereDate('end_date', '<', $start_date);
////        $check = $check->whereDate('start_date', '<=', $start_date)->get();
//dd($check);
////        2022-06-27 10:00:00
////        $check = $check->where('end_date', '<', $start_date)->get();
//
////            ->where('start_date', '<=', $start_date)->get();
