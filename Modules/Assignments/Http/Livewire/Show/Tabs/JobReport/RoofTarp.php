...
class RoofTarp extends Component
{
    ...
    public function mount(Assignment $assignment, AssignmentsJobTypes $job_type)
    {
        ...
        $this->reportsDB = JobReportReports::where('assignment_id',$this->assignment->id)->where('job_type_id', 1)->pluck('report_option_id');
        $this->workersDB = JobReportWorkers::where('assignment_id',$this->assignment->id)->where('job_type_id', 1)->pluck('worker_id');
        ...
        if(is_null($this->jobReport)){
            ...
            //  $this->reportsDB =  JobReportReports::where('assignment_id',$this->assignment->id)->where('job_type_id', 1)->pluck('id');
    ...
    {
        return view('assignments::livewire.show.tabs.job-report.roof-tarp', [
